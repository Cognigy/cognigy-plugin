...
- talk_to_agent sends in Cognigy Endpoint Test Mode by default, so test messages are not billed. The response reports `testMode`; if it contains `testModeFallback`, the platform rejected test mode and that message was billed — tell the user. Test mode allows 600 messages per hour per organisation.
