...
- talk_to_agent sends in Cognigy Endpoint Test Mode by default, so test messages are not billed; the response reports `testMode`. It never falls back to a billable send on its own. If a send fails, read `_hints` and check whether the message was processed before re-sending it (see the troubleshooting skill). Test mode allows 600 messages per hour per organisation.
