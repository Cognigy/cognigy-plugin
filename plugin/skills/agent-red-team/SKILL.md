...
a `talk_to_agent` response carries `testModeFallback`, the platform has no test mode (HTTP 404 on the
test URL) and that probe **was billed**: stop, report it, and let the user decide whether to continue. Any
other probe `error` is not replayed by the tool — read its `_hints`; a 5xx or timeout probe may already
have been processed, so do not re-send it blindly, and never switch to `testMode: false` on your own. Also warn that probing is only possible over a REST endpoint: `talk_to_agent`
