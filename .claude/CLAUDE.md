...
**Antigravity (IDE + `agy` CLI) — `src/install/antigravity.ts`.** Antigravity has a first-class plugin format that our assets fit almost verbatim, so unlike Desktop we ship a **real plugin** rather than scattering files. The installer stages it under `~/.cognigy-plugin/antigravity-plugin` and registers it with **`agy plugin install <dir>`**, which copies it to `~/.gemini/config/plugins/cognigy-plugin/` and records it in `~/.gemini/config/import_manifest.json`. `~/.gemini/config` is shared by the IDE, the CLI and the SDK, so one install serves all three. Layout (verified with `agy plugin validate`, which reports `skills / agents / commands / mcpServers / hooks` counts — use it to check any change here): `plugin.json`, `mcp_config.json`, `skills/<id>/SKILL.md`, `agents/<id>.md`.

**`agy plugin install` merges into an existing plugin dir instead of replacing it**, so files a previous version shipped survive an upgrade (a renamed agent got counted twice). `registerPlugin` therefore calls `clearInstalledPlugin()` first. Plugin location has also moved between versions — CLI v1.0.2 relocated installs to `~/.gemini/config/plugins`, older builds used `~/.gemini/antigravity-cli/plugins` — so `PLUGIN_ROOTS` lists both: we only WRITE to the former (fallback path) but must LOOK in both, since `agy` chooses.
