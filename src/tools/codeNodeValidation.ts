...
/** Bare identifier call — not preceded by `.` so `obj.fetch(` is not matched. */
const bareCall = (name: string) => new RegExp(`(?<![.\\w$])${name}\\s*\\(`);
...
function checkHttp(code: string, errors: string[]): void {
  ...
  if (bareCall("fetch").test(code) || /\bXMLHttpRequest\b/.test(code)) {
    errors.push(
...
function checkModuleLoading(code: string, errors: string[]): void {
  const hasRequire = bareCall("require").test(code);
  const hasImport =
    /^\s*import\s+(?:[\w*{}\s,$]+\s+from\s+)?["']/m.test(code) ||
    bareCall("import").test(code);
