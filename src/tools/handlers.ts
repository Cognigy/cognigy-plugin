  ...
  async handleManageFlowNodes(args: any): Promise<any> {
    ...
    switch (operation) {
      ...
      case "render": {
        ...
        const baseAction =
          'CRITICAL — how to deliver the diagram so it renders natively (zoomable) instead of as a plain, broken-looking code block (this matters most on mobile): the ```mermaid fenced block MUST be the ONLY content of its message. No preamble (no "Here\'s the flow:"), no trailing text, and DO NOT put the `ascii` tree or the `legend` in that same message. Clients only promote a mermaid block to the interactive viewer when the message is just that block; anything else alongside it forces a fallback to a plain code block. Put any commentary, and (only if useful) the `legend` as a short list, in a SEPARATE message before or after the diagram. The `ascii` tree is ONLY a fallback for clients that cannot render mermaid at all (e.g. a terminal); if the client renders mermaid, do not show the ascii tree next to it.';
