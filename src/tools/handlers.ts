  ...
  async handleManageFlowNodes(args: any): Promise<any> {
    ...
    switch (operation) {
      ...
      case "render": {
        ...
        const baseAction =
          "CRITICAL — how to deliver the diagram so it renders natively (zoomable, mobile-friendly): create a Mermaid ARTIFACT whose content is EXACTLY the `mermaid` string (an artifact of mermaid/diagram type, titled e.g. Flow: <flow name>). Only an artifact produces the native diagram viewer with a 'Download and open' card that works well on phones. Do NOT paste the mermaid as an inline ```mermaid fenced code block — inline renders as plain text or a tiny thumbnail (the broken-looking result, especially on mobile). Keep ALL other content — any commentary, the `legend`, the `ascii` tree — OUTSIDE the artifact, in the normal message text. The `ascii` tree is only a fallback for clients with no artifact/mermaid support at all (e.g. a terminal).";
