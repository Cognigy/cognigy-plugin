...
After creating, use talk_to_agent to test.`,
  ...
  // 15. manage_settings
  {
    ...
    description:
      'Manage project-level settings in Cognigy, including Voice Preview Settings and Knowledge AI Settings.\n\nBEFORE USING THIS TOOL: Read cognigy://guide/settings for the full workflow.\n\nOperations:\n- set_voice_preview: Configure the speech provider for voice preview. Requires projectId and provider. If connectionId is omitted, the tool auto-detects an existing speech connection for the chosen provider. If no connection is found, it returns instructions to upload a package containing one via manage_packages.\n- set_knowledge_ai: Configure Knowledge AI Settings for the project. For normal AI-agent knowledge-store setups, use this to set the Knowledge Search model and the Content Parser. answerExtractionModelId is also supported, but is usually not needed for AI-agent knowledge-store setups. Model IDs must be llm_model referenceIds from the SAME project. These are NOT the embedding-model settings used by manage_knowledge for knowledge-store indexing. If you provide a model ID, the tool automatically enables generative AI settings for the project. If contentParser is "azure", azureDIConnectionId is required.\n\nSupported speech providers: microsoft, google, aws, deepgram, elevenlabs.',
    annotations: {
