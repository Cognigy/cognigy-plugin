    ...
    inputSchema: {
      ...
      properties: {
        ...
        provider: {
          ...
          description:
            "LLM provider (API values: 'openAI', 'azureOpenAI', 'anthropic', 'google', 'mistral', 'openAICompatible', 'awsBedrock', 'googleGenAI'). Use 'openAICompatible' for any endpoint that speaks the OpenAI API (vLLM, Hugging Face, LiteLLM, Azure AI Foundry, ...); 'awsBedrock' for models on AWS Bedrock; 'googleGenAI' for Gemini 3+ on Vertex AI.",
        },
        modelType: {
          type: "string",
          description:
        ...
        location: {
          type: "string",
          description:
            "googleGenAI only (required): Vertex AI region, e.g. 'us-central1' or 'global'.",
        },
        serviceAccountJson: {
          type: "string",
          description:
            "googleGenAI only: the full GCP service-account JSON key as a string (must contain client_email, private_key, project_id). A GoogleVertexAIProvider Connection is auto-created from it.",
        },
