...
export const tools: ToolDefinition[] = [
  ...
  // 4. talk_to_agent
  {
    name: "talk_to_agent",
    description:
      "Send a message to a Cognigy AI Agent and get its response. Use this to test agent behavior during iterative development.\n\nTwo modes:\n1. DIRECT: Provide endpointUrl (from create_ai_agent or list_resources { resourceType: 'endpoint' }).\n2. BY AGENT: Provide aiAgentId — the tool automatically finds or creates a REST endpoint for the agent's flow.\n\nUse the same sessionId across calls for multi-turn conversations.\n\nBILLING: messages are sent in Cognigy Endpoint Test Mode by default (the /test/<token> URL variant), which keeps them out of the customer's billable conversation count. Test mode is capped at 600 messages per hour per organisation, so keep automated runs (e.g. red-team probes) within that budget. If the test-mode URL answers 404 (the platform predates Cognigy 4.27 and has no /test/ route), the message is re-sent to the regular endpoint and the response carries testMode: false, a testModeFallback object and a _hints.warning — surface that to the user, because that message was billed. Any other failure is returned as an error WITHOUT re-sending: a 5xx or timeout may already have been processed by the agent, and 400/403/429 have causes unrelated to test mode. Read _hints before deciding, together with the user, whether to re-send with testMode: false.\n\nReturns: agentResponse text, sessionId, the endpointUrl actually used and testMode. Add verbose: true for the full raw API response.",
    annotations: {
      title: "Talk to Agent",
      readOnlyHint: false,
      destructiveHint: true,
    ...
    inputSchema: {
      ...
      properties: {
        ...
        testMode: {
          type: "boolean",
          description:
            "Send via Cognigy Endpoint Test Mode (default: true) so the message is not billed. Set false only when the user explicitly wants a real, billable production message, or after a test-mode failure has been diagnosed (via _hints) as unsupported test mode and the user accepts the billing.",
        },
      },
      required: ["message"],
    },
