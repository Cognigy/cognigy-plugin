...
describe('update_tool', () => {
  ...
  it('updates config for mcp type (mcpName, mcpServerUrl, timeout)', async () => {
    ...
    const result = await h.handleToolCall('update_tool', {
      ...
      toolNodeId: ID.tool,
      toolType: 'mcp',
      config: { mcpName: 'remote-mcp', mcpServerUrl: 'https://mcp.example.com', timeout: 60 },
    });
    ...
    expect(api.patch).toHaveBeenCalledWith(
      `/v2.0/flows/${ID.flow}/chart/nodes/${ID.tool}`,
      { config: { name: 'remote-mcp', mcpServerUrl: 'https://mcp.example.com', timeout: 60 } },
    );
  });
  ...
  it('updates HTTP child node when http config fields present', async () => {
    ...
    const result = await h.handleToolCall('update_tool', {
      aiAgentId: ID.agent,
      toolNodeId: ID.tool,
      config: { url: 'https://api.example.com', method: 'POST', headers: { 'X-Key': 'val' }, body: '{}' },
    });
    ...
    expect(api.patch).toHaveBeenCalledWith(
      ...
      { config: expect.objectContaining({ url: 'https://api.example.com', type: 'POST' }) },
    );
  });
