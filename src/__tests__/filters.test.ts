...
describe('filterResponse', () => {
  ...
  it('webchat3 summary picks up persistentMenu from layout.enablePersistentMenu', () => {
    ...
    expect(filterResponse('endpoint', raw).webchatSummary.persistentMenu).toBe(true);
  });
...
describe('filterList', () => {
  ...
  it('filters each item in array', () => {
    const items = [
      { _id: 'p1', name: 'P1', description: 'd1', createdAt: '2024-01-01', extra: 'x' },
      { _id: 'p2', name: 'P2', description: 'd2', createdAt: '2024-02-01', extra: 'y' },
    ];
  ...
  it('applies correct filter per resource type', () => {
    ...
    const conversations = [
      { sessionId: 's1', channel: 'rest', startedAt: '2024-01-01', messageCount: 3, messages: [] },
    ];
    ...
    expect(filterList('conversation', conversations)).toEqual([
      { sessionId: 's1', channel: 'rest', startedAt: '2024-01-01', messageCount: 3 },
    ]);
  });
});
